...
/**
 * The kill switch. Reachable in one tap from every header; this screen is
 * where that tap lands. No further confirmation once the button here is
    ...
    <Screen>
      ...
      <View className="flex-1 items-center px-gutter-lg pt-6">
        ...
        <Text className="mt-6 text-center font-strong text-headline text-ink">
          {store.paused ? 'Everything is paused' : 'Pause everything?'}
        </Text>
        <Text className="mt-2.5 text-center font-body text-body-sm text-slate">
          {store.paused
            ? 'The assistant cannot propose or run a payment while this is on. Every allowance stays exactly where it is.'
            : 'The assistant stops proposing and running payments immediately. Nothing else changes — your allowances and their balances stay exactly where they are.'}
        </Text>
      </View>

      <ActionBar>
        <Button
          label={working ? 'Working' : store.paused ? 'Resume everything' : 'Pause everything'}
