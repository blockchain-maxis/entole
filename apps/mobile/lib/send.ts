import { countryFor } from '@entole/core/countries';
...
/**
 * What the amount screens check before they let someone press Review. The
 * per-send limits are the server's: at least $1.00 and at most $10,000, so in
 * the account's own currency they move with the rate. The fee is deliberately
 * not here — it is only known from a quote, and the confirmation sheet handles
 * "not enough once the fee is added".
 */
...
export type AmountCheck =
  ...
  | {
       ...
       * quiet about the first and leads to adding money for the second. */
...
const NOT_FOR_PEOPLE = /\b(wallet|crypto|blockchain|chain|gas|token|on-chain|signature|transaction hash)\b|0x[0-9a-f]{6,}|https?:\/\//i;
...
export type CountryGroup = {
  /** ISO alpha-2, or null for people whose country has not been set. */
  code: string | null;
  name: string;
  contacts: Contact[];
};

/** Beneficiaries grouped by the country they are in — alphabetical, with the
 * ones whose country is not set last. Sending abroad starts from this. */
export function groupByCountry(contacts: readonly Contact[]): CountryGroup[] {
  const groups = new Map<string, CountryGroup>();
  for (const contact of contacts) {
    const country = countryFor(contact.place);
    const key = country?.code ?? '';
    const group = groups.get(key) ?? {
      code: country?.code ?? null,
      name: country?.name ?? 'Country not set',
      contacts: [],
    };
