...
import { hasOnboarded, markOnboarded, reauthenticate, registerAccount, sessionIsFresh } from '@/lib/session';
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-[560px] flex-col items-center justify-center px-gutter-lg">
      ...
      <button
        type="button"
        disabled={busy}
        onClick={() => void (onboarded ? signIn() : onboard())}
      ...
      >
        {busy ? 'Confirming' : onboarded ? 'Sign in with passkey' : 'Continue with passkey'}
      </button>
